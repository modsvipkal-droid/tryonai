...
export async function syncLatestGames() {
  ...
  const result = await db.collection("results_30s").bulkWrite(
    games.map((game) => ({
      updateOne: {
...
export async function queryGames(options = {}) {
  ...
  const collection = db.collection("results_30s");
...
export async function getGameCacheStats() {
  ...
  const latest = await db
    .collection("results_30s")
    .find({}, { projection: { _id: 0, period: 1, syncedAt: 1, blockTimestamp: 1 } })
    .sort({ blockTimestamp: -1 })
    .limit(1)
    .next();
  const total = await db.collection("results_30s").estimatedDocumentCount();
