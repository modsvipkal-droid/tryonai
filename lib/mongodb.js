...
const MONGODB_DB = process.env.MONGODB_DB || "wingo_db";
...
export async function ensurePaymentIndexes() {
  ...
  try {
    const db = await getMongoDb();
    // Legacy rows stored literal `null` in utr/gateway_order_id. Sparse-unique
    // indexes still index explicit nulls, so purge them first — otherwise index
    // creation (and the second payment order) fails with duplicate-key 11000.
    ...
    await db.collection("payments").updateMany(
      ...
      { $unset: { gateway_order_id: "" } }
    );
    await Promise.all([
      db.collection("payments").createIndex({ order_id: 1 }, { unique: true }),
      db.collection("payments").createIndex({ gateway_order_id: 1 }, { unique: true, sparse: true }),
      db.collection("payments").createIndex({ utr: 1 }, { unique: true, sparse: true }),
      db.collection("payments").createIndex({ status: 1, created_at: -1 }),
    ]);
    paymentIndexesReady = true;
  } catch (e) {
    console.error("[mongodb] ensurePaymentIndexes error:", e?.message);
  }
}
