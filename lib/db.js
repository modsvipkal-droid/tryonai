...
// ──── Subscription screen anti-abuse (server-side enforcement) ────────────
// The counter and block flags live on the user document and are mutated with a
// single atomic pipeline update so multiple tabs/devices can never race past
// the limit. Client storage is never trusted.

export const SUBSCRIPTION_VISIT_LIMIT = 5;
export const SUBSCRIPTION_BLOCK_REASON = `${SUBSCRIPTION_VISIT_LIMIT} subscription screen visits without purchase`;

/** Server-side purchase check: only verified payments (or an admin grant) count. */
export function hasVerifiedPurchase(user) {
  if (!user) return false;
  if (user.hasSuccessfulPurchase === true) return true;
  if (Array.isArray(user.model_access) && user.model_access.length > 0) return true;
  if (user.unlimited === true) return true;
  return false;
}

export function isSubscriptionBlocked(user) {
  return !!user && user.subscriptionBlocked === true;
}

/**
 * Atomically increment `subscriptionScreenVisits` for an unpaid account and,
 * when the limit is reached, set subscriptionBlocked/blockReason/blockedAt in
 * the SAME document operation — safe under concurrent requests.
 */
export async function recordSubscriptionVisit(email, visitId) {
  const now = Date.now();
  if (hasMongo()) {
    try {
      const db = await getMongoDb();
      const result = await db.collection("users").findOneAndUpdate(
        ...
        [
          {
            $set: {
              __nextVisits: { $add: [{ $ifNull: ["$subscriptionScreenVisits", 0] }, 1] },
              __shouldBlock: {
                $and: [
                  { $ne: [{ $ifNull: ["$subscriptionBlocked", false] }, true] },
                  { $eq: [{ $ifNull: ["$hasSuccessfulPurchase", false] }, false] },
                  { $eq: [{ $ifNull: ["$unlimited", false] }, false] },
                  { $not: [{ $gt: [{ $size: { $cond: [{ $isArray: "$model_access" }, "$model_access", []] } }, 0] }] },
                  { $gte: [{ $add: [{ $ifNull: ["$subscriptionScreenVisits", 0] }, 1] }, SUBSCRIPTION_VISIT_LIMIT] },
                ],
              },
            },
          },
          {
            $set: {
              subscriptionScreenVisits: "$__nextVisits",
              subscriptionBlocked: { $cond: ["$__shouldBlock", true, { $ifNull: ["$subscriptionBlocked", false] }] },
              blockReason: { $cond: ["$__shouldBlock", SUBSCRIPTION_BLOCK_REASON, { $ifNull: ["$blockReason", null] }] },
              blockedAt: { $cond: ["$__shouldBlock", now, { $ifNull: ["$blockedAt", null] }] },
              lastVisitId: String(visitId || ""),
            },
          },
          { $unset: ["__nextVisits", "__shouldBlock"] },
        ],
        { returnDocument: "after", includeResultMetadata: false }
      );
      return toPlain(result);
    } catch (e) { console.error("recordSubscriptionVisit mongo error:", e?.message); }
    return null;
  }
  // JSON fallback (single-process dev environment).
  const users = readJSON(USERS_FILE);
  const idx = users.findIndex((u) => u.email === email);
  if (idx === -1) return null;
  const u = users[idx];
  const nextVisits = (u.subscriptionScreenVisits || 0) + 1;
  const purchased =
    u.hasSuccessfulPurchase === true ||
    u.unlimited === true ||
    (Array.isArray(u.model_access) && u.model_access.length > 0);
  if (!purchased && nextVisits >= SUBSCRIPTION_VISIT_LIMIT && u.subscriptionBlocked !== true) {
    u.subscriptionBlocked = true;
    u.blockReason = SUBSCRIPTION_BLOCK_REASON;
    u.blockedAt = now;
  }
  u.subscriptionScreenVisits = nextVisits;
  u.lastVisitId = String(visitId || "");
  users[idx] = u;
  writeJSON(USERS_FILE, users);
  return u;
}
