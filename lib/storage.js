...
const PREDICTIONS_KEY = 'trx_today_predictions';
  ...
  try {
    ...
    if (data.found) {
      ...
      if (user) {
        ...
        user.unlimitedAt = data.unlimited ? Date.now() : null;
        if (data.fx1) {
          user.fx1_subscription = data.fx1;
        }
        saveUsers(users);
      }
    }
...
export async function getProfilePlan(email) {
  try {
    const res = await fetch(`/api/users/check?email=${encodeURIComponent(email)}`);
    const data = await res.json();
    if (data.found) {
      ...
      const user = users.find(u => u.email === email);
      if (user) {
        user.model = (data.model || "").toLowerCase();
        if (data.fx1) user.fx1_subscription = data.fx1;
        saveUsers(users);
      }
  ...
  const users = getUsers();
  return users.find(u => u.email === email)?.model?.toLowerCase() || "";
}
...
export async function isUnlimited(email) {
  try {
    ...
    const users = getUsers();
    const user = users.find(u => u.email === email);
    // Server decides using server time — auto-expiry is authoritative.
    if (data.unlimited) {
      if (user) {
        user.unlimited = true;
  ...
  const users = getUsers();
  return users.find(u => u.email === email)?.unlimited || false;
}
...
function getAllTodayPredictions() {
  try {
    const data = JSON.parse(localStorage.getItem(PREDICTIONS_KEY) || '{}');
    const today = data[getTodayKey()] || {};
    return today;
  } catch { return {}; }
}

function saveAllTodayPredictions(today) {
  try {
    const data = JSON.parse(localStorage.getItem(PREDICTIONS_KEY) || '{}');
...
export function getTodayPredictions(email) {
  return getAllTodayPredictions()[email] || [];
}

export function addTodayPrediction(email, prediction) {
  const today = getAllTodayPredictions();
  const list = today[email] || [];
  list.push(prediction);
  today[email] = list;
  saveAllTodayPredictions(today);
  return list;
}
