...
export async function signInWithGoogle() {
  try {
    ...
    console.error("Firebase sign-in failed, using email fallback:", err?.message);
  }

  let email = "";
  for (let attempts = 0; attempts < 3; attempts++) {
    email = window.prompt("Apna email daalein (Enter your email):");
    if (!email) {
      if (attempts < 2) continue;
      throw { code: "cancelled-popup", message: "Email required" };
    }
  ...
  if (!email) throw { code: "cancelled-popup", message: "Email required" };
