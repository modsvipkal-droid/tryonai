...
async function createSession(firebaseUser) {
  try {
    ...
    // Server-side block enforcement: surface it to the caller instead of
    // silently continuing so the login screen can show the block message.
    if (res.status === 403) {
    ...
    if (e?.code === "auth/account-blocked") throw e;
  ...
  try {
    ...
    if (auth) {
      ...
      unsub = onAuthStateChanged(auth, async (firebaseUser) => {
        if (firebaseUser) {
          // A server-side blocked account must look like "not signed in".
          let blocked = false;
          try {
            await createSession(firebaseUser);
          } catch (e) {
            if (e?.code === "auth/account-blocked") blocked = true;
          }
          callback(blocked ? null : firebaseUser);
        } else {
          callback(null);
        }
      });
